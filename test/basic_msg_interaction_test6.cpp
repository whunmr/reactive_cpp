...
#define __step(func) sync_calls_.push_back(makeMIStep<func>());

#define MI_ACK_MSGS MIData<outer_mi>::ack_msgs_

#define msg_ack_handler_name(msg)     msg_##_handler
#define msg_ack_handler_instance(msg) msg_##_handler_##_instance
...
do {                                                                             \
  ...
  } msg_ack_handler_instance(MSG);						 \
  Dispatcher::inst().registerAction( msg_ack_handler_instance(MSG).msg_id	 \
				   , msg_ack_handler_instance(MSG));		 \
  MIData<outer_mi>::ack_msgs_.insert(msg_ack_handler_instance(MSG).msg_id);      \
} while(0)
...
struct __out_most_mi {
};

struct __nested_mi_at_level_1 {
};

struct Basic_mi_c : BaseMI {
  Basic_mi_c() { mi_declare_steps_flow(); }
  typedef __out_most_mi outer_mi;
  ...
  /*-----------------------------------------------*/
	...
	Status ret =  async_call_1(ctxt, msg);
        /*-----------------------------------------------*/
	on_msg_ack(msg_ack_1, on_msg_ack_1);
	on_msg_ack(msg_ack_2, on_msg_ack_2);
	on_msg_ack(msg_ack_3, on_msg_ack_3);
        /*-----------------------------------------------*/
	
	static struct msg_ack_4_handler : MsgIdWrapper<msg_ack_4>
					, AckMsgAction<do_remove_ack_msgs> {
	  typedef __nested_mi_at_level_1 outer_mi;
          virtual Status do_handle_msg(const Msg& msg) {
	    //////////////////////////////////////
		...
		static struct nested_async_call_1_handler : MIStep /*add guardTask.*/ {
		  ...
		  virtual Status handle_msg(MIContext& ctxt, const Msg& msg) {
		    ...
		    /*-----------------------------------------------*/
		    on_msg_ack(nested_msg_ack_1, on_nested_msg_ack_1);
		    on_msg_ack(nested_msg_ack_2, on_nested_msg_ack_2);
		    /*-----------------------------------------------*/
